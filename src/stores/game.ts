...
export const useGameStore = defineStore('game', () => {
  ...
  async function loadCharacterTraits() {
    try {
      ...
      if (error) {
        log('加载角色特性失败：' + error.message)
        return
      }
      charTraits.value = Object.fromEntries((data ?? []).map((r: any) => [r.name, {
  ...
  async function loadUserDeckFromDB(uid: string) {
    ...
    if (error) { log('加载玩家手牌失败：' + error.message); return }
    const cards: Card[] = (data ?? []).flatMap((r: any) => {
  ...
  async function loadEnemyDeck(stageNum: number) {
    ...
    const { data, error } = await supabase.from('enemy_cards').select('*').eq('stage', stageNum).eq('difficulty', diff)
    if (error) { log('加载敌方手牌失败：' + error.message); return }
    enemyHand.value = (data ?? []).map((r: any) => ({
      id: uid2(),
      name: r.name,
      cost: r.type === 'spell' ? 2 : r.type === 'equipment' ? 3 : 2,
      type: r.type,
