...
| 探索与战斗 | `stages` | 模板 | 关卡模板：`stage_number`、`chapter_number`、`dungeon_id`、`is_boss_stage` |
| 探索与战斗 | `dungeons` | 模板 | 地牢模板：`difficulty`(easy/normal/hard/expert)、`recommended_cards` |
| 探索与战斗 | `runs` | 实例 | 探索记录：`stage_number`、`chapter_number`、`preparation_snapshot`、`result` |
| 探索与战斗 | `user_stage_progress` | 实例 | 关卡进度：`stage_number`、`is_passed`、`is_unlocked`、`attempt_count` |
| 探索与战斗 | `enemies` | 模板 | 敌人设定：`difficulty`、`base_stats`、`behavior_script` |
| 探索与战斗 | `enemy_cards` | 关联 | 敌人卡组：`enemy_id`、`card_id`、`weight` |
| 探索与战斗 | `events` | 配置 | 随机事件：`location_type`(camp/dungeon)、`effect_payload`、`trigger_condition` |
...
| 统计与洞察 | `user_achievements` | 实例 | 用户成就：`user_id`、`achievement_id`、`is_completed`、`progress`、`completed_at`、`reward_claimed` |
...
> - **地牢（Dungeon）**：关卡的地牢模板，定义地牢的基本属性
> - **探索流程**：进入关卡 → 地牢探索 → 触发事件/战斗 → 完成关卡

**`stages`**（关卡模板库）  
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` (PK) | BIGINT | 关卡ID（自增主键） |
| `stage_number` | INT | **关卡编号（1, 2, 3...）** |
| `chapter_number` | INT | **章节编号（每5关为一章：1, 2, 3...）** |
| `dungeon_id` | BIGINT | 关联的地牢模板ID（FK→dungeons.id） |
| `stage_name` | VARCHAR | 关卡名称 |
| `difficulty` | VARCHAR | 难度：easy/normal/hard/expert |
| `is_boss_stage` | TINYINT(1) | **是否为Boss关卡（每章第5关）** |
| `enemy_pool` | JSON | **敌人池配置**（用于随机生成敌人） |
| `event_pool` | JSON | **事件池配置**（用于随机生成事件） |
| `reward_pool` | JSON | **奖励池配置**（关卡完成奖励） |
| `exploration_map` | JSON | **探索地图配置**（房间、路径、探索点） |
| `description` | TEXT | 关卡描述 |
| UNIQUE KEY uk_stage_number (stage_number) | | 关卡编号唯一 |

> **关卡编号规则**：
> - 关卡编号从1开始，连续递增
> - 章节编号 = `CEIL(stage_number / 5)`
> - 每章第5关（5, 10, 15...）为Boss关卡
> - 例如：第1-5关为第1章，第6-10关为第2章
...
| `name` | VARCHAR | 地牢名称 |
| `difficulty` | VARCHAR | 难度等级：easy/normal/hard/expert |
| `recommended_cards` | JSON | 推荐卡牌（JSON数组） |
| `description` | TEXT | 地牢描述 |
| `theme` | VARCHAR | 地牢主题（如：森林、洞穴、废墟等） |

**`runs`**（探索记录实例）  
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` (PK) | BIGINT | 探索记录ID（自增主键） |
| `user_id` | BIGINT | 用户ID（FK→users.id） |
| `user_player_character_id` | BIGINT | 玩家角色实例ID（FK→user_player_characters.id） |
| `stage_number` | INT | **当前关卡编号** |
| `chapter_number` | INT | **当前章节编号** |
| `dungeon_id` | BIGINT | 地牢ID（FK→dungeons.id） |
| `preparation_snapshot` | JSON | 准备快照（手牌、道具、货币） |
| `current_stage_progress` | JSON | **关卡内进度**（探索进度、已触发事件、已击败敌人） |
| `result` | VARCHAR | 探索结果：victory-胜利, defeat-失败, abandon-放弃 |
| `started_at` | TIMESTAMP | 开始时间 |
| `ended_at` | TIMESTAMP | 结束时间 |
| `reward_snapshot` | JSON | 奖励快照 |

> **关卡内进度说明**：
> - `current_stage_progress` 存储JSON格式的进度数据
> - 包含：`explored_rooms`（已探索房间）、`triggered_events`（已触发事件）、`defeated_enemies`（已击败敌人）、`current_room`（当前房间）等

**`user_stage_progress`**（用户关卡进度）  
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` (PK) | BIGINT | 进度ID（自增主键） |
| `user_id` | BIGINT | 用户ID（FK→users.id） |
| `stage_number` | INT | **关卡编号** |
| `chapter_number` | INT | **章节编号** |
| `is_passed` | TINYINT(1) | **是否已通过** |
| `is_unlocked` | TINYINT(1) | **是否已解锁**（通过前一关后解锁） |
| `best_result` | VARCHAR | 最佳结果：victory/defeat |
| `passed_at` | TIMESTAMP | 首次通过时间 |
| `attempt_count` | INT | 尝试次数 |
| `best_score` | INT | 最佳评分（可选） |
| UNIQUE KEY uk_user_stage (user_id, stage_number) | | 用户+关卡唯一 |

> **关卡解锁规则**：
> - 第1关默认解锁
> - 通过第N关后，自动解锁第N+1关
> - 通过第5关（Boss关）后，解锁下一章的第1关
...
| `name` | VARCHAR | 事件名称 |
| `location_type` | VARCHAR | 事件类型：camp-营地, dungeon-地牢 |
| `trigger_condition` | JSON | **触发条件**（关卡编号范围、章节、房间类型等） |
| `trigger_chance` | DECIMAL(3,2) | **触发概率**（0.00-1.00，地牢事件使用） |
| `description` | TEXT | 事件描述 |
| `effect_payload` | JSON | **效果参数**（奖励、惩罚、选择项等） |
| `choices` | JSON | **选择项配置**（如果有多个选择） |

> **事件触发机制**：
> - **地牢内事件**：在探索过程中随机触发
> - **触发条件**：根据 `trigger_condition` 判断（如：`{"stage_range": [1, 10], "chapter": 1}`）
> - **触发概率**：根据 `trigger_chance` 随机判断（如：0.3 表示30%概率）
> - **事件效果**：根据 `effect_payload` 执行（如：`{"reward": {"gold": 100}, "stress": 10}`）
> - **事件选择**：如果有 `choices`，玩家可以选择不同选项，产生不同效果
...
| `id` (PK) | BIGINT | 实例ID（自增主键） |
| `user_id` (FK→users.id) | BIGINT | 用户ID |
| `achievement_id` (FK→achievements.id) | BIGINT | 成就ID |
...
| UNIQUE KEY uk_user_achievement (user_id, achievement_id) | | 用户+成就唯一 |

> **设计说明**：遵循项目的"模板/实例分离"设计原则。`achievements` 表存储成就定义（所有用户共用），`user_achievements` 表存储用户成就完成状态（每个用户独立）。支持成就进度追踪、完成时间记录、奖励领取状态管理。
...
6. **选择关卡**  
   - 玩家在营地选择要挑战的关卡
   - 检查 `user_stage_progress.is_unlocked` 判断是否解锁
   - 显示关卡信息：名称、难度、推荐卡牌、奖励预览

7. **进入关卡（地牢）**  
   - 创建 `runs` 记录，记录 `stage_number`、`chapter_number`
   - 生成 `preparation_snapshot`，包含手牌、道具、货币余量
   - 初始化 `current_stage_progress`（探索进度、已触发事件等）
   - 根据 `stages.exploration_map` 生成地牢地图

8. **地牢内探索**  
   - **探索模式**：玩家可以在地牢中移动、探索房间
   - **触发事件**：
     - 根据 `events.trigger_condition` 和 `trigger_chance` 随机触发事件
     - 事件类型为 `location_type: dungeon`
     - 执行事件效果（奖励、惩罚、选择等）
     - 记录到 `runs.current_stage_progress.triggered_events`
   - **触发战斗**：
     - 根据 `stages.enemy_pool` 随机生成敌人
     - 进入战斗模式
     - 战斗胜利/失败后返回探索模式
     - 记录到 `runs.current_stage_progress.defeated_enemies`

9. **战斗模式**  
   - 回合制卡牌战斗
   - 使用玩家手牌和敌人卡组
   - 战斗结果：victory（胜利）、defeat（失败）
   - 战斗后更新角色状态（血量、压力等）

10. **完成关卡**  
    - 达成关卡目标（如：击败Boss、到达终点）
    - 更新 `user_stage_progress.is_passed = 1`
    - 解锁下一关（`user_stage_progress.is_unlocked = 1`）
    - 发放奖励（根据 `stages.reward_pool`）
    - 更新 `runs.result` 和 `ended_at`
...
5. 创建数据库迁移脚本，确保平滑升级。
6. **实现关卡系统**：
   - 创建 `stages` 表和 `user_stage_progress` 表
   - 实现关卡解锁逻辑
   - 实现章节系统（每5关为一章）
7. **实现地牢探索流程**：
   - 实现探索模式UI
...
- ✅ **新增关卡系统设计**
  - 新增 `stages` 表（关卡模板）
  - 新增 `user_stage_progress` 表（用户关卡进度）
  - 实现章节系统（每5关为一章）
  - 实现关卡解锁机制
- ✅ **完善地牢探索流程**
  - 探索模式设计
