...
- **技能图片支持**：
  1. ✅ 支持为每个技能添加自定义图片
  2. ✅ 图片存储路径：`/static/images/skills/{职业代码}/{技能code}.png`
  3. ✅ 如果图片不存在，自动回退到CSS样式图标
  4. ✅ 图片会显示在技能图标容器内，保持边框和光效效果
  5. 📄 详细说明请查看：[技能图片存储方案.md](./frontend/技能图片存储方案.md)
- **改进建议**：
  1. ✅ 已支持技能图标图片显示
  2. 可以添加技能预览功能，显示技能效果
  3. 可以优化连接线的绘制算法，支持更复杂的技能树结构

## ♻️ 技能系统战斗实现分析与修复（2026-01-06）
- **产出**：
  1. **技能系统全面分析报告**：
     - ✅ 分析了前后端功能（skills.vue 和 SkillController.java）
     - ✅ 分析了数据库设计（player_characters、skills、user_player_character_skills 表）
     - ✅ 评估了战斗时主动和被动技能效果的实现可行性
     - ✅ 检查了技能设计的合理性（玩家职业没有攻击力，只有生命和法力值）
  2. **关键发现**：
     - ✅ **数据库完全支持**：技能效果存储在 `effect_payload` JSON字段中，解锁记录在 `user_player_character_skills` 表中
     - ✅ **战斗系统支持动态修改敌人攻击力**：`BattleContext` 有 `setEnemyAttack()` 方法，已扩展支持百分比减少
     - ⚠️ **战斗系统未集成**：战斗系统目前只使用了卡牌效果和角色特性，技能效果完全没有在战斗中使用
     - ⚠️ **技能设计问题**：9个技能有 `attack_bonus` 或 `attack_speed_bonus` 效果，但玩家职业没有攻击力字段
  3. **实现可行性**：
     - ✅ **完全可行**：数据库和项目架构完全支持技能效果实现
     - ⚠️ **需要改进**：需要创建技能效果执行服务，并在战斗系统中集成技能效果
  4. **Service设计优化**：
     - ✅ **合并SkillEffectService到SkillService**：SkillEffectService只是薄薄的包装层，没有独立复杂逻辑，合并后更符合单一职责原则
     - ✅ **SkillService现在包含**：数据管理、解锁逻辑、效果执行（技能相关的所有业务逻辑）
  5. **创建文件**：
     - `技能系统战斗实现分析报告.md`：详细的分析报告
     - `技能系统战斗实现深度分析与修复方案.md`：深度分析和修复方案
     - `技能Service职责分析与合并方案.md`：Service职责分析和合并说明
     - `database/fix_skill_attack_bonus.sql`：SQL修复脚本，修复9个问题技能
     - `技能系统修复完成总结.md`：任务完成总结
- **产出**：
  1. **技能系统全面分析报告**：
     - ✅ 分析了前后端功能（skills.vue 和 SkillController.java）
     - ✅ 分析了数据库设计（player_characters、skills、user_player_character_skills 表）
  ...
  2. **关键发现**：
     - ✅ **数据库完全支持**：技能效果存储在 `effect_payload` JSON字段中，解锁记录在 `user_player_character_skills` 表中
     - ⚠️ **战斗系统未集成**：战斗系统目前只使用了卡牌效果和角色特性，技能效果完全没有在战斗中使用
     - ⚠️ **技能设计问题**：部分技能有 `attack_bonus` 效果，但玩家职业没有攻击力字段，这些效果无法应用到玩家身上
  3. **实现可行性**：
     - ✅ **完全可行**：数据库和项目架构完全支持技能效果实现
     - ⚠️ **需要改进**：需要创建技能效果执行服务，并在战斗系统中集成技能效果
  4. **创建文件**：
     - `技能系统战斗实现分析报告.md`：详细的分析报告，包含前后端功能分析、数据库设计分析、技能效果实现可行性分析、技能设计合理性分析和实现建议
- **问题**：
  1. 技能效果未在战斗中使用，技能解锁后无法在战斗中使用
  2. 部分技能有 `attack_bonus` 效果，但玩家职业没有攻击力，这些效果不合理
  3. 缺少技能效果执行服务，虽然有 `CardEffectService`，但没有专门的技能效果执行服务
- **改进建议**：
  1. ✅ **已完成**：扩展CardEffectService支持百分比减少敌人攻击力（`attack_reduction`、`enemy_attack_reduction`）
  2. ✅ **已完成**：创建技能效果执行服务（`SkillEffectService`），支持主动和被动技能
  3. ✅ **已完成**：创建SQL修复脚本（`database/fix_skill_attack_bonus.sql`），修复9个问题技能
  4. ⏳ **待实现**：执行SQL修复脚本更新数据库
  5. ⏳ **待实现**：在战斗系统中集成被动技能（自动触发）
  6. ⏳ **待实现**：实现主动技能使用逻辑和前端技能使用界面
- **创建文件**：
  - `技能系统战斗实现深度分析与修复方案.md`：详细的分析报告，包含战斗系统分析、技能问题分析、修复方案
  - `database/fix_skill_attack_bonus.sql`：SQL修复脚本，修复9个问题技能
  - `backend/src/main/java/com/dungeon/service/SkillEffectService.java`：技能效果执行服务
  - `技能系统修复完成总结.md`：任务完成总结
