  ...
  <div class="explore-container">
    ...
    <div class="explore-content">
      ...
      <main class="main-section">
        ...
        <div class="map-card">
          ...
          >
            ...
            <!-- 节点层 -->
            <div class="route-nodes" :style="{ width: `${mapWidth}px`, height: `${mapHeight}px` }">
              <div
            ...
            >
                <!-- 分支节点容器 -->
                <div v-if="node.type === 'branch'" class="branch-container">
                  <!-- 左分支 -->
                  <div class="branch-node branch-left">
                    <!-- 战斗节点 -->
                    <div
                    ...
                    >
                      ...
                      <div class="node-number">{{ node.leftLevel }}</div>
                      <div class="node-status">
                        <span v-if="levelStatus(node.leftLevel).claimed" class="status-badge claimed">✓</span>
                        <span v-else-if="levelStatus(node.leftLevel).passed" class="status-badge passed">✓</span>
                        <span v-else-if="levelStatus(node.leftLevel).isCurr" class="status-badge current">●</span>
              </div>
                      <div class="node-label">战斗</div>
                    </div>
                    <!-- 未知节点 -->
                <div
                ...
                >
                      <div class="node-icon unknown-icon">❓</div>
                      <div class="node-label">未知</div>
                      <div class="unknown-glow"></div>
                  </div>
                </div>
                  <!-- 右分支 -->
                  <div class="branch-node branch-right">
                    <!-- 战斗节点 -->
                    <div
                    ...
                    >
                      <div class="node-icon">⚔️</div>
                      <div class="node-number">{{ node.rightLevel }}</div>
                      <div class="node-status">
                        <span v-if="levelStatus(node.rightLevel).claimed" class="status-badge claimed">✓</span>
                        <span v-else-if="levelStatus(node.rightLevel).passed" class="status-badge passed">✓</span>
                        <span v-else-if="levelStatus(node.rightLevel).isCurr" class="status-badge current">●</span>
              </div>
                      <div class="node-label">战斗</div>
                    </div>
                    <!-- 未知节点 -->
                ...
                <!-- 普通节点 -->
                <div v-else-if="node.level" class="main-node">
                  <div
                    class="route-node"
                  ...
                  >
                    <div class="node-icon">⚔️</div>
                    <div class="node-number">{{ node.level }}</div>
                    <div class="node-status">
                      <span v-if="levelStatus(node.level).claimed" class="status-badge claimed">✓</span>
                      <span v-else-if="levelStatus(node.level).passed" class="status-badge passed">✓</span>
                      <span v-else-if="levelStatus(node.level).isCurr" class="status-badge current">●</span>
                    </div>
                    <div class="node-difficulty">{{ stageInfo(node.level).difficulty }}</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <aside class="stage-detail-panel" :class="{ 'panel-visible': selectedStageLevel || isUnknownNode }">
        <div class="panel-content" v-if="selectedStageLevel || isUnknownNode">
          <button class="panel-close" @click="closeStageDetail">✕</button>
          
          <!-- 关卡标题 -->
          <div class="stage-title-section">
            <div class="stage-combat-icon">{{ isUnknownNode ? '❓' : '⚔️' }}</div>
            <div class="stage-title-info">
              <div class="stage-combat-label">{{ isUnknownNode ? '奇遇' : '作战' }}</div>
              <div class="stage-name-label">{{ isUnknownNode ? '未知事件' : getStageInfo(selectedStageLevel!).name }}</div>
          </div>
            </div>

          <!-- INFO 描述框 -->
          <div class="stage-info-box">
            <div class="info-box-label">INFO</div>
          ...
          <!-- 预览区域 - 奇遇节点时不显示 -->
          <div class="stage-preview-section" v-if="!isUnknownNode">
            <!-- 敌方情报 -->
            <div class="preview-item">
              ...
              <div class="preview-content enemy-preview">
                <div class="preview-placeholder">
                  <div class="preview-icon-large">👹</div>
                  <div class="preview-text">敌人信息</div>
        </div>
              </div>
    </div>

            <!-- 通关奖励 -->
            <div class="preview-item" v-if="selectedStageLevel">
              <div class="preview-label">
                <span class="preview-icon">🎁</span>
                <span>通关奖励</span>
          </div>
              <div class="preview-content reward-preview">
            <div class="reward-list">
              <div class="reward-item">
                    <div class="reward-icon">🪙</div>
                    <div class="reward-info">
                      <div class="reward-label">金币</div>
                      <div class="reward-value">{{ getRewards(selectedStageLevel, getStageInfo(selectedStageLevel).difficulty).gold }}</div>
                    </div>
              </div>
              <div class="reward-item">
                    <div class="reward-icon">⭐</div>
                    <div class="reward-info">
                      ...
                      <div class="reward-value">{{ getRewards(selectedStageLevel, getStageInfo(selectedStageLevel).difficulty).exp }}</div>
              </div>
            </div>
          </div>
          </div>
          ...
          <!-- 底部操作按钮 -->
          <div class="stage-action-section">
            <button 
              ...
              :disabled="inBattle"
            >
              <span class="start-btn-icon">{{ isUnknownNode ? '❓' : '⚔️' }}</span>
              <span class="start-btn-text">{{ isUnknownNode ? '前往' : '開戰' }}</span>
              <span class="start-btn-subtext">{{ isUnknownNode ? '×奇遇' : '×作战' }}</span>
            </button>
          </div>
        </div>
      </aside>
    ...
    <!-- 奇遇事件弹窗 -->
    <div v-if="showEventModal && currentEvent" class="event-modal-backdrop" @click.self="closeEventModal">
      <div class="event-modal">
        <div class="event-modal-header">
          <span class="event-modal-icon">❓</span>
          <div class="event-modal-title">{{ currentEvent.title }}</div>
...
import { campApi, stageProgressApi, stageApi } from '@/lib/api'
...
.explore-container {
  min-height: 100vh;
  background: url('/tansuo.png') center center / cover no-repeat;
