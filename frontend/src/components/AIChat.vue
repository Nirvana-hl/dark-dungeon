...
<template>
  <div class="fixed right-4 bottom-4 z-40">
    <div class="card rounded-2xl w-80 overflow-hidden shadow-lg" v-show="open">
      <div class="status-bar px-4 py-2 flex items-center justify-between">
        <div class="font-semibold">AI 助手</div>
        <button class="action-button rounded-button px-2 py-1" @click="open = false">收起</button>
      </div>
      <div class="p-3 h-64 overflow-y-auto space-y-2" ref="listEl">
        <div v-for="(m,i) in messages" :key="i" class="text-sm" :class="m.role === 'user' ? 'text-right' : 'text-left'">
          <span class="inline-block px-3 py-2 rounded-lg" :class="m.role === 'user' ? 'bg-indigo-600' : 'bg-slate-700'">{{ m.text }}</span>
        </div>
      </div>
