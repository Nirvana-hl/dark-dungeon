      ...
      <main class="modal-content">
        <div class="offer-showcase">
          ...
          <div class="offer-image-large">
            ...
            />
            <div v-if="hasDiscount" class="discount-badge-large">
          ...
          <div class="offer-details">
            <div class="offer-meta">
              <div class="rarity-badge" :class="offer.rarity">
            ...
            <div class="offer-description-full">
              <h3>商品描述</h3>
              <p>{{ offer.description }}</p>
            </div>

            <!-- 卡牌属性（如果是卡牌） -->
            <div v-if="isCard" class="card-attributes">
              <h3>属性详情</h3>
              <div class="attributes-grid">
                <div v-if="cardStats.attack" class="attribute-item">
                  <i class="fas fa-sword attribute-icon attack"></i>
                  <div class="attribute-info">
                    <span class="attribute-name">攻击力</span>
                    <span class="attribute-value">{{ cardStats.attack }}</span>
                  </div>
                </div>
                <div v-if="cardStats.health" class="attribute-item">
                  <i class="fas fa-heart attribute-icon health"></i>
                  <div class="attribute-info">
                    <span class="attribute-name">生命值</span>
                    <span class="attribute-value">{{ cardStats.health }}</span>
                  </div>
                </div>
                <div v-if="cardStats.actionPointCost" class="attribute-item">
                  <i class="fas fa-bolt attribute-icon ap"></i>
                  <div class="attribute-info">
                    <span class="attribute-name">行动点消耗</span>
                    <span class="attribute-value">{{ cardStats.actionPointCost }}</span>
                  </div>
                </div>
                <div v-if="cardTraits.length > 0" class="attribute-item">
            ...
            <!-- 使用效果（如果是道具） -->
            <div v-if="isItem" class="item-effects">
              <h3>使用效果</h3>
              <div class="effects-list">
                <div class="effect-item">
                  <i class="fas fa-plus effect-icon heal"></i>
                  <span>恢复 50 点生命值</span>
                </div>
              </div>
            </div>

            <!-- 礼包内容（如果是礼包） -->
            <div v-if="isBundle" class="bundle-contents">
              <h3>礼包内容</h3>
        ...
        <div class="purchase-section">
          <div class="price-info">
            ...
            <div class="current-price">
              ...
              <div class="price-amount">
                ...
                <span class="amount">{{ formatAmount(offer.price) }}</span>
              </div>
          ...
          <!-- 购买数量选择 -->
          <div v-if="allowQuantity" class="quantity-selector">
            <label>购买数量:</label>
          ...
          <!-- 库存信息 -->
          <div v-if="offer.stock !== undefined" class="stock-status">
            <span class="stock-label">剩余库存:</span>
            <span class="stock-count" :class="{ 'low-stock': offer.stock <= 10 }">
              {{ offer.stock }}
            </span>
          </div>
        </div>
