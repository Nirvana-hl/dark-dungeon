<template>
  <div class="character-panel">
    <!-- 主角卡片 -->
    <div class="character-card premium">
      <div class="character-header">
        ...
        <div class="character-info">
          ...
          <div class="character-title">{{ getCharacterTitle(1) }}</div>
        </div>
      </div>
      ...
      <div class="core-stats">
        <div class="stat-item hp">
          ...
          <div class="stat-content">
            <div class="stat-bar">
              <div class="stat-fill hp-fill" :style="{ width: hpPercentage + '%' }"></div>
            </div>
            <span class="stat-value">{{ playerCharacter?.currentHp || 0 }}/{{ playerCharacter?.maxHp || 100 }}</span>
          </div>
        </div>
        ...
        <div class="stat-item ap">
          ...
          <div class="stat-content">
            <div class="stat-bar">
              <div class="stat-fill ap-fill" :style="{ width: apPercentage + '%' }"></div>
            </div>
            <span class="stat-value">{{ playerCharacter?.currentActionPoints || 0 }}/{{ playerCharacter?.maxActionPoints || 5 }}</span>
          </div>
        </div>
        
        <div class="stat-item stress" :class="stressLevelClass" @click="$emit('showStressDetails')">
          <div class="stat-icon"><i class="fas fa-brain"></i></div>
          <div class="stat-content">
            <div class="stat-bar">
              <div class="stat-fill stress-fill" :style="{ width: stressPercentage + '%' }"></div>
            </div>
            <span class="stat-value">{{ playerCharacter?.currentStress || 0 }}% (Lv.{{ playerCharacter?.stressLevel || 1 }})</span>
          </div>
        </div>
      </div>
      ...
      <div class="quick-actions">
        <RouterLink to="/skills" class="action-btn primary">
          <i class="fas fa-graduation-cap"></i>
          <span>技能树</span>
          <span class="action-count">{{ unlockedSkills.length }}/{{ totalSkills }}</span>
        </RouterLink>
        <button class="action-btn secondary" @click="$emit('manageCards')">
          <i class="fas fa-layer-group"></i>
          <span>卡组</span>
          <span class="action-count">{{ equippedCards.length }}</span>
        </button>
      </div>
    </div>
  </div>
