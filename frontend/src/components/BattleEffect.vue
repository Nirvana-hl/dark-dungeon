  ...
  <Transition name="effect-fade">
    ...
    >
      <div class="effect-content">
        <div class="effect-icon">{{ config.icon }}</div>
        <div v-if="value > 0" class="effect-value" :style="{ color: config.color }">
          {{ type === 'damage' || type === 'critical' ? '-' : '+' }}{{ value }}
        </div>
        <div v-if="type === 'critical'" class="effect-label">{{ config.label }}</div>
      </div>
    </div>
  </Transition>
</template>
