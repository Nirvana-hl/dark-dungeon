...
// 在页面挂载时主动从后端加载营地数据（包括 userCards）
onMounted(async () => {
  try {
    console.log('[CardsPage] 页面挂载，开始从后端加载营地数据...')
    await camp.fetchCampData()
    console.log('[CardsPage] 营地数据加载完成，userCards count=', (camp.userCards || []).length)

  // 加载当前装备状态，如果内存中没有数据才从数据库获取
  if (!game.initialDeck || game.initialDeck.length === 0) {
    console.log('[CardsPage] 内存中没有装备数据，从数据库获取')
    await game.loadUserDeckFromDB()
    console.log('[CardsPage] 装备状态加载完成，equipped count=', (game.initialDeck || []).length)
  } else {
    console.log('[CardsPage] 使用内存中的装备数据，equipped count=', game.initialDeck.length)
  }
  } catch (err) {
    console.error('[CardsPage] 加载营地数据失败:', err)
  }
})
...
async function handleEquipCard(card: any) {
  ...
  console.log('[CardsPage] handleEquipCard called, card=', card)

  // 优先尝试持久化到后端，再更新本地 state
  const isCharacterType = (card.cardType === 'character') || (card.raw && (card.raw.cardType === 'character' || card.raw.cardCharacterId))
  ...
  try {
    if (isCharacterType) {
      ...
      console.log('[CardsPage] deploying character to camp:', userCardCharacterId)
      ...
      if (charIndex >= 0 && camp.userCardCharacters && camp.userCardCharacters[charIndex]) {
        ...
        console.log('[CardsPage] manually updated character deployment status to true')
      }

      // 创建装备卡牌对象并立即添加到装备列表（立即UI更新）
      console.log('[CardsPage] 创建角色装备卡前的原始数据:', {
        cardId: card.id,
        cardName: card.name,
        rawData: card.raw,
        statModifiers: card.raw?.statModifiers,
        effect: card.raw?.effect
      })
      const equippedCard = createEquippedCard(card)
      console.log('[CardsPage] 创建的角色装备卡数据:', {
        id: equippedCard.id,
        name: equippedCard.name,
        bonusAttack: equippedCard.bonusAttack,
        bonusHp: equippedCard.bonusHp,
        bonusDefense: equippedCard.bonusDefense
      })
      game.initialDeck = [...(game.initialDeck ?? game.deck ?? []), equippedCard]
      game.deck = [...(game.deck ?? []), equippedCard]
      console.log('[CardsPage] immediately added character to equipped deck, new length=', game.initialDeck?.length)

      // 异步调用后端API和同步状态
      try {
        const characterEquipResponse = await campApi.deployCardCharacter(userCardCharacterId, true)
        console.log('[CardsPage] character equip API response:', characterEquipResponse)
        await camp.fetchCampData()
        console.log('[CardsPage] character equip synced with backend')
      } catch (err) {
        console.error('[CardsPage] backend sync failed, reverting local changes')
        // 如果后端失败，回滚本地更改
      ...
      console.log('[CardsPage] updating userCard loadout:', userCardId, '->', loadoutId)
      ...
      if (cardIndex >= 0 && camp.userCards && camp.userCards[cardIndex]) {
        ...
        console.log('[CardsPage] manually updated card loadoutId in raw data')
      }

      // 创建装备卡牌对象并立即添加到装备列表（立即UI更新）
      console.log('[CardsPage] 创建普通装备卡前的原始数据:', {
        cardId: card.id,
        cardName: card.name,
        rawData: card.raw,
        statModifiers: card.raw?.statModifiers,
      ...
      const equippedCard = createEquippedCard(card)
      console.log('[CardsPage] 创建的普通装备卡数据:', {
        id: equippedCard.id,
        name: equippedCard.name,
        bonusAttack: equippedCard.bonusAttack,
        bonusHp: equippedCard.bonusHp,
      ...
      game.initialDeck = [...(game.initialDeck ?? game.deck ?? []), equippedCard]
      game.deck = [...(game.deck ?? []), equippedCard]
      console.log('[CardsPage] immediately added card to equipped deck, new length=', game.initialDeck?.length)

      // 异步调用后端API和同步状态
      try {
        const cardEquipResponse = await userCardApi.updateUserCard(userCardId, { loadoutId })
        console.log('[CardsPage] card equip API response:', cardEquipResponse)
        await camp.fetchCampData()
        console.log('[CardsPage] card equip synced with backend')
      } catch (err) {
        console.error('[CardsPage] backend sync failed, reverting local changes')
        // 如果后端失败，回滚本地更改
    ...
    console.error('[CardsPage] error while equipping card', e)
  }

  // 减少用户卡牌数量（乐观更新）
  try {
    camp.decrementUserCardQuantity(card.id, 1)
    console.log('[CardsPage] decremented card quantity in store for id=', card.id)
  } catch (e) {
    console.warn('[CardsPage] failed to decrement card quantity', e)
  }
}
...
function createEquippedCard(card: any): any {
  ...
  if (isCharacterType) {
    ...
    return {
      ...
      name: rawData.characterName || '营地角色',
      cost: 2,
...
async function handleUnequipCard(payload: any) {
  ...
  console.log('[CardsPage] handleUnequipCard called, id=', id, 'rawId=', rawId)
  ...
  try {
    if (isCharacterType) {
      ...
      console.log('[CardsPage] undeploying character:', userCardCharacterId)
      ...
      if (charIndex >= 0 && camp.userCardCharacters && camp.userCardCharacters[charIndex]) {
        camp.userCardCharacters[charIndex].isDeployed = false
        console.log('[CardsPage] manually updated character deployment status to false')
      }
      ...
      console.log('[CardsPage] immediately removed character from equipped deck')
      ...
      try {
        ...
        console.log('[CardsPage] character unequip API response:', characterUnequipResponse)
        await camp.fetchCampData()
        console.log('[CardsPage] character unequip synced with backend')
      } catch (err) {
        console.error('[CardsPage] backend sync failed, reverting local changes')
        // 如果后端失败，回滚本地更改
        if (charIndex >= 0 && camp.userCardCharacters && camp.userCardCharacters[charIndex]) {
      ...
      console.log('[CardsPage] updating userCard remove from loadout:', targetId)
      console.log('[CardsPage] unequip card raw data:', payload.raw)
      // 确保使用正确的后端ID
      const actualTargetId = payload.raw?.cardId ?? payload.raw?.userCardId ?? payload.raw?.id ?? targetId
      console.log('[CardsPage] actual target ID for unequip:', actualTargetId)
      ...
      if (cardIndex >= 0 && camp.userCards && camp.userCards[cardIndex]) {
        ...
        console.log('[CardsPage] manually updated card loadoutId to null in raw data')
      }
    ...
    const match = (c: any) => {
      ...
      game.initialDeck = (game.initialDeck ?? []).filter(c => !match(c))
      game.deck = (game.deck ?? []).filter(c => !match(c))
      console.log('[CardsPage] immediately removed card from equipped deck')

      // 异步调用后端API（撤下操作不重新加载装备状态，避免状态被覆盖）
      try {
        const unequipResponse = await userCardApi.updateUserCard(actualTargetId, { loadoutId: null })
        console.log('[CardsPage] unequip API response:', unequipResponse)
        await camp.fetchCampData()
        console.log('[CardsPage] card unequip synced with backend')
      } catch (err) {
        console.error('[CardsPage] backend sync failed, reverting local changes')
        // 如果后端失败，回滚本地更改
        if (cardIndex >= 0 && camp.userCards && camp.userCards[cardIndex] && camp.userCards[cardIndex].raw) {
    ...
    console.error('[CardsPage] persist unequip failed:', err)
    // 如果后端同步失败，尝试回滚本地数量恢复
    try {
  ...
  try {
    ...
    console.log('[CardsPage] incremented card quantity in store for id=', restoreId, 'insertIndex=', originalIndex, 'idVariants=', idVariants)
  } catch (e) {
    console.warn('[CardsPage] failed to increment card quantity', e)
  }
}
</script>
