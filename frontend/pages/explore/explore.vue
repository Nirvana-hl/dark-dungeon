      ...
      <view class="main-section">
        ...
        <view class="map-card">
          <scroll-view 
            ...
            scroll-y
            :style="`height: ${mapScrollHeight}px`"
          ...
          >
            ...
            <!-- 节点层 -->
              <view class="route-nodes">
                <!-- 门动画（基于第5关位置） -->
                <view :style="doorWrapperStyle" class="door-wrapper" @click.stop="handleDoorClick">
...
// 当前开发阶段只支持两层（第一层：1-5关，第二层：6-10关）
const maxLevel = 15
...
const routeNodes = computed<RouteNode[]>(() => {
  try {
    // 添加安全检查，确保依赖的计算属性已初始化
    const currentChapter = chapter.value || 1
    const startLevel = chapterStartLevel.value || 1
    const endLevel = chapterEndLevel.value || 5

    // 构建层数组：根据章节不同生成不同的分支布局
    // 默认布局（多数章节使用）：
    const defaultLayers: Array<Array<Partial<RouteNode>>> = [
      [{ type: 'main', level: startLevel }],                 // layer 0 (bottom) - 本章首关
      // layer1: triple (unknown, main(2), unknown)
      [{ type: 'unknown' }, { type: 'main', level: startLevel + 1 }, { type: 'unknown' }],
      // layer2: unknown + main(3)
      [{ type: 'unknown' }, { type: 'main', level: startLevel + 2 }],
      // layer3: triple (unknown, main(4), unknown)
      [{ type: 'unknown' }, { type: 'main', level: startLevel + 3 }, { type: 'unknown' }],
      [{ type: 'main', level: endLevel }]                     // layer 4 (top) - 本章末关
    ]

    // 特殊布局：第二章（chapter === 2）按要求：
    // - 第3关（indexInChapter=3）旁边有两个奇遇（两侧各一个）
    // - 第2关与第4关旁边只有一个奇遇
    const chapter2Layers: Array<Array<Partial<RouteNode>>> = [
      [{ type: 'main', level: startLevel }],                 // layer0: 首关
      // layer1: single unknown + main(2)
      [{ type: 'unknown' }, { type: 'main', level: startLevel + 1 }],
      // layer2: unknown + main(3) + unknown (两侧各一个奇遇)
      [{ type: 'unknown' }, { type: 'main', level: startLevel + 2 }, { type: 'unknown' }],
      // layer3: unknown + main(4)
      [{ type: 'unknown' }, { type: 'main', level: startLevel + 3 }],
      [{ type: 'main', level: endLevel }]                     // layer4: 末关
    ]

    // 第三章（chapter === 3）仅包含主线节点（无奇遇）
    const chapter3Layers: Array<Array<Partial<RouteNode>>> = [
      [{ type: 'main', level: startLevel }],
      [{ type: 'main', level: startLevel + 1 }],
      [{ type: 'main', level: startLevel + 2 }],
      [{ type: 'main', level: startLevel + 3 }],
      [{ type: 'main', level: endLevel }]
    ]

    let layers = defaultLayers
    if (currentChapter === 2) layers = chapter2Layers
    else if (currentChapter === 3) layers = chapter3Layers
...
const mapHeight = computed(() => {
  ...
  const calc = nodes.length * nodeSpacing + nodeOffset * 2
  try {
...
// 固定可滚动区域高度，避免使用 vh 导致移动端地址栏收缩时高度抖动
const mapScrollHeight = ref<number>(0)
...
onLoad((options?: Record<string, any>) => {
  ...
  // 计算并锁定 map scroll 高度（只在加载时计算一次）
  try {
    const sysInfo = typeof (uni as any).getSystemInfoSync === 'function'
      ? (uni as any).getSystemInfoSync()
      : { windowHeight: 800 }
    mapScrollHeight.value = Math.round((sysInfo.windowHeight || 800) * 0.68)
  } catch (e) {
    mapScrollHeight.value = 600
  }
...
// 探索页面整体背景：第一章使用 tansuo1.png，第二章使用 tansuo2.png（仅支持两章）
const exploreBgStyle = computed(() => {
  try {
    ...
    if (chapter.value === 2) {
      return {
        backgroundImage: "url('/static/tansuo2.png')",
        backgroundSize: 'cover',
        backgroundRepeat: 'no-repeat',
        backgroundPosition: 'center top'
      } as any
...
// 开发阶段控制：是否通过点击门进入下一层（true），发布后改为自动跳转（false）
const devPortalMode = ref(true)

// 找到本章末关的像素位置，用于把门定位在其上方居中
const doorNodeIndex = computed(() => {
  const nodes = routeNodes.value || []
  const endLevel = chapterEndLevel.value || 5
  return nodes.findIndex(n => n.level === endLevel)
})
const doorPixel = computed(() => {
  const nodes = routeNodes.value || []
  const currentMapHeight = mapHeight.value || 800
  if (doorNodeIndex.value < 0) return { x: mapWidth / 2, y: currentMapHeight / 2 }
  const p = getNodePixelPosition(nodes[doorNodeIndex.value], doorNodeIndex.value)
  return p ?? { x: mapWidth / 2, y: currentMapHeight / 2 }
})
...
const doorWrapperStyle = computed(() => {
  ...
  const top = (p.y || (currentMapHeight / 2)) - doorHeight - 24
  return {
    position: 'absolute',
    left: `${left}px`,
    top: `${top}px`,
    ...
    // 开发阶段允许点击传送门；发布时设置为 none 并由通关后自动跳转
    pointerEvents: devPortalMode.value ? 'auto' : 'none',
    cursor: devPortalMode.value ? 'pointer' : 'default',
    zIndex: 1200
  } as any
})
...
watch(mapHeight, (h) => {
  try {
    // 不再主动设置 scrollTop，避免程序性滚动影响用户体验。
    // 标记为已初始化以保留原有初始化语义（但不修改滚动位置）
    if (!autoScrollInitialized.value) autoScrollInitialized.value = true
  } catch (e) {
    if (!autoScrollInitialized.value) autoScrollInitialized.value = true
  }
})
...
watch(passedLevels, (newArr) => {
  try {
    if (newArr.includes(chapterEndLevel.value) && !doorOpen.value && !inBattle.value && !isDragging.value) {
      ...
      setTimeout(() => {
        // 开发阶段：门只做动画并等待点击；发布后：自动跳转到下一章
        if (!devPortalMode.value) {
          // 自动进入下一章（如果存在）
          try {
            gotoNextChapterAuto()
          } catch (e) { console.error('[Explore] auto goto next chapter failed', e) }
        }
      }, 1200)
    }
  } catch (e) {
...
function getNodePixelPosition(node: RouteNode, index: number): { x: number, y: number } | null {
  ...
  const nodes = routeNodes.value || []
  const numLayers = nodes.length > 0 ? Math.max(1, Math.max(...nodes.map(n => n.layer || 0)) + 1) : 1
  const topGap = nodesTopGap.value || 100
  const currentMapHeight = mapHeight.value || 800
  const bottomY = currentMapHeight - nodeOffset
...
// 点击/跳转到下一章（开发阶段人工点击传送门）
function gotoNextChapterManual() {
  if (inBattle.value) return
  const nextStart = chapterStartLevel.value + 5
  if (nextStart > maxLevel) return
  // 设置当前关卡到下一章的首关
  currentLevel.value = nextStart
  selectedStageLevel.value = nextStart
  // 重新生成路线/边线，重置滚动初始化
  routeEdgesRef.value = []
  autoScrollInitialized.value = false
  regenerateRouteEdgesForChapter()
  // 关闭门动画以避免重复点击
  doorOpen.value = false
  // 切章时强制绘制一次，确保线条与节点对齐
  setTimeout(() => {
    try { drawRoutesOnCanvas() } catch (e) { console.error('[Explore] draw on chapter manual error', e) }
    try { updateSVGPaths() } catch (e) { console.error('[Explore] updateSVGPaths on chapter manual error', e) }
  }, 120)
}

// 自动跳转（通关后使用）
function gotoNextChapterAuto() {
  if (inBattle.value) return
  const nextStart = chapterStartLevel.value + 5
  if (nextStart > maxLevel) return
  // 切换当前章节首关并刷新进度显示
  ...
  regenerateRouteEdgesForChapter()
  doorOpen.value = false
  // 切章时强制绘制一次，确保线条与节点对齐（自动模式）
  setTimeout(() => {
    try { drawRoutesOnCanvas() } catch (e) { console.error('[Explore] draw on chapter auto error', e) }
    try { updateSVGPaths() } catch (e) { console.error('[Explore] updateSVGPaths on chapter auto error', e) }
  }, 120)
}
...
// ---- 重绘节流器（避免频繁触发导致内存/CPU 均增长） ----
let __rafHandle: number | null = null
let __fallbackTimer: number | null = null
const rafAvailable = typeof requestAnimationFrame === 'function'

function scheduleRedraw() {
  // 如果已有 RAF/定时器正在排队，则不重复排队
  if (__rafHandle != null || __fallbackTimer != null) return
  if (rafAvailable) {
    __rafHandle = requestAnimationFrame(() => {
      try {
        drawRoutesOnCanvas()
        updateSVGPaths()
      } catch (e) {
        console.error('[Explore] scheduled redraw error', e)
      } finally {
        __rafHandle = null
      }
    })
    ...
    // 退回到定时器，每 100ms 最多一次
    __fallbackTimer = setTimeout(() => {
      try {
        drawRoutesOnCanvas()
        updateSVGPaths()
      } catch (e) {
        console.error('[Explore] scheduled redraw (timeout) error', e)
      } finally {
        if (__fallbackTimer != null) {
          clearTimeout(__fallbackTimer as any)
          __fallbackTimer = null
        }
      }
...
function cancelScheduledRedraw() {
  try {
    if (__rafHandle != null) {
      if (typeof cancelAnimationFrame === 'function') cancelAnimationFrame(__rafHandle as number)
      __rafHandle = null
    }
    if (__fallbackTimer != null) {
      clearTimeout(__fallbackTimer as any)
...
// 只在首次准备好时绘制一次（"一次定死"），之后停止 watcher，避免后续任何自动重绘
const stopOnceDraw = watch([routeEdgesRef, routeNodes, mapHeight], () => {
  try {
    drawRoutesOnCanvas()
    updateSVGPaths()
  } catch (e) {
    console.error('[Explore] one-time draw error', e)
  } finally {
    try { stopOnceDraw() } catch (e) {}
    // 取消任何已排队的计划重绘，确保不会有后续绘制
    cancelScheduledRedraw()
  }
}, { immediate: true })

// 当滚动位置变化时也重绘，保证线条跟随节点（固定相对位置）
// 不再响应滚动重绘（一次定死策略），保留空实现以防外部调用依赖
// watch(scrollTop, () => {})
