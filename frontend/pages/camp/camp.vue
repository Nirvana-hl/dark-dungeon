  ...
  <view class="camp-official">
    ...
    <view class="main-content">
      ...
      <view class="content-area">
        <!-- 卡组管理 -->
        <view v-if="activeTab === 'cards'" class="module-card">
          ...
          >
            ...
            <view v-if="cardCharacters.length === 0" class="card-characters-empty">
              <i class="fas fa-user-slash"></i>
              <p>尚未拥有角色卡牌，前往商城购买即可解锁。</p>
            </view>

            <view v-else class="card-characters-grid">
              <view 
              ...
              >
                <view class="character-card-header">
                  <text class="rarity-tag" :class="getCharacterRarityClass(character)">
                    {{ getCharacterRarityLabel(character.characterRarity) }}
                  </text>
                  <text class="quantity-tag">×{{ character.quantity ?? 0 }}</text>
                  <button
                ...
                <view class="character-card-body">
                  ...
                  <view class="character-stats">
                    <text>
                      ...
                      <!-- 如果升星后血量有提升，显示提升提示 -->
                      <text v-if="character.currentHp && character.baseHp && character.currentHp > character.baseHp" 
                            class="stat-bonus" 
                    ...
                    <text>
                      ...
                      <text v-if="character.currentStarLevel && character.currentStarLevel > 1" 
                            class="stat-bonus" 
          ...
          >
            ...
            <view v-if="spellCards.length === 0" class="card-characters-empty">
            ...
            <view v-else class="card-characters-grid">
              ...
              >
                <view class="character-card-header">
                  <text class="rarity-tag" :class="normalizeRarity(card.rarity || 'common')">
                    {{ getRarityName(normalizeRarity(card.rarity || 'common')) }}
                  </text>
                  <text class="quantity-tag">×{{ card.quantity || 1 }}</text>
                </view>
                <view class="character-card-body">
                  <view class="character-name">
          ...
          >
            ...
            <view v-if="equipmentCards.length === 0" class="card-characters-empty">
        ...
        <!-- 背包与道具 -->
        <view v-if="activeTab === 'inventory'" class="module-card">
          <!-- 物品分类 -->
          ...
          <!-- 物品列表 -->
          <view class="items-enhanced-list">
            <view 
        ...
        <!-- 商城 -->
        <view v-if="activeTab === 'shop'" class="module-card">
          <ShopPanel 
        ...
        <!-- AI助手 -->
        <view v-if="activeTab === 'ai'" class="module-card">
      ...
      <view class="ai-suggestions">
        ...
        >
          <view class="suggestion-header">
            ...
            <text class="suggestion-priority" :class="suggestion.priority">
              {{ suggestion.priority }}
            </text>
          </view>
          <view class="suggestion-content">
            <p>{{ suggestion.content }}</p>
            <view class="suggestion-data" v-if="suggestion.data">
              <h5>数据分析:</h5>
              <ul>
      ...
      <view class="ai-metrics">
        ...
        <view class="metrics-grid">
          ...
          >
            ...
            <text class="metric-change" :class="metric.trend">
    ...
    <!-- 压力详情弹窗 -->
    <view v-if="showStressModal" class="stress-modal-overlay" @click="showStressModal = false">
      <view class="stress-modal-container" @click.stop>
        ...
        <main class="stress-modal-body">
          <view class="stress-summary-grid" v-if="stressStatus">
            <view class="stress-summary-card">
              <text class="label">当前等级</text>
              <text class="value">Lv.{{ stressStatus.stressLevel }}</text>
            </view>
            <view class="stress-summary-card">
              <text class="label">当前压力值</text>
              <text class="value">{{ stressStatus.currentStress }} / 100</text>
            </view>
          </view>

          <view class="stress-levels-grid">
            <view 
            ...
            >
              <view class="level-header">
                <text class="level-tag">等级 {{ level }}</text>
              ...
              <view class="debuff-list" v-if="stressDebuffsByLevel[level] && stressDebuffsByLevel[level].length">
                <view 
                ...
                >
                  <view class="debuff-main">
                    <view class="debuff-name">{{ debuff.debuffName }}</view>
                    <view class="debuff-desc">{{ debuff.effectDescription }}</view>
                  </view>
                  <view class="debuff-extra">
                    <text class="chance" v-if="debuff.triggerChance">
                      {{ Math.round(debuff.triggerChance * 100) }}%
                    </text>
                  </view>
                </view>
              </view>
              <view class="debuff-empty" v-else>
                暂无配置
              </view>
            </view>
          </view>
...
import { campApi, stressApi, userCardApi, userCardCharacterApi } from '@/api/request'
...
onLoad(async () => {
  try {
    ...
    showNotification('error', errorMessage, 'fas fa-exclamation-triangle')
    console.error('[CampOfficial] 营地页面初始化失败:', {
...
.logo-section {
  ...
  flex-direction: column;
  gap: 8rpx;
  margin-left: 200rpx; /* 为返回首页按钮留出空间，避免重叠 */
}
...
.camp-title {
  ...
  margin: 0;
  text-shadow: 0 0 30rpx rgba(255, 215, 0, 0.3);
}
...
.prompt-icon {
  ...
  filter: drop-shadow(0 0 20rpx rgba(212, 175, 55, 0.5));
}
...
.create-character-btn:hover {
  ...
  box-shadow: 0 16rpx 40rpx rgba(212, 175, 55, 0.6);
  background: linear-gradient(135deg, #ffd700, #ffed4e);
}
...
.card-3d:hover {
  transform: translateY(-16rpx) rotateX(5deg);
}
...
.card-frame.rare { border-color: #2196f3; box-shadow: 0 0 40rpx rgba(33, 150, 243, 0.3); }
.card-frame.epic { border-color: #9c27b0; box-shadow: 0 0 40rpx rgba(156, 39, 176, 0.3); }
.card-frame.legendary { 
  border-color: #ff9800; 
  box-shadow: 0 0 60rpx rgba(255, 152, 0, 0.5);
  background: linear-gradient(145deg, rgba(255, 152, 0, 0.2), rgba(255, 152, 0, 0.05));
}
...
.card-3d.equipped {
  ...
  box-shadow: 0 16rpx 50rpx rgba(76, 175, 80, 0.4);
}
...
.character-card-mini.rarity-rare {
  ...
  box-shadow: 0 24rpx 60rpx rgba(33, 150, 243, 0.25);
}

.character-card-mini.rarity-epic {
  border-color: rgba(156, 39, 176, 0.5);
  box-shadow: 0 24rpx 60rpx rgba(156, 39, 176, 0.3);
}

.character-card-mini.rarity-legendary {
  border-color: rgba(255, 152, 0, 0.7);
  box-shadow: 0 30rpx 70rpx rgba(255, 152, 0, 0.35);
...
@keyframes pulse-glow {
  ...
  50% {
    opacity: 0.8;
    text-shadow: 0 0 16rpx rgba(76, 175, 80, 0.8);
  }
}
...
.character-status.standby {
  ...
  color: #d1d5db;
  border: 2rpx solid rgba(156, 163, 175, 0.3);
}
...
.items-enhanced-list {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(640rpx, 1fr));
  gap: 1rem;
}
...
.product-card.empty {
  ...
  border: 4rpx dashed rgba(255, 255, 255, 0.1);
  display: flex;
  align-items: center;
  justify-content: center;
  min-height: 560rpx;
...
.quest-icon-bg.daily { background: rgba(33, 150, 243, 0.2); border: 2rpx solid rgba(33, 150, 243, 0.4); }
.quest-icon-bg.weekly { background: rgba(156, 39, 176, 0.2); border: 2rpx solid rgba(156, 39, 176, 0.4); }
.quest-icon-bg.special { background: rgba(255, 152, 0, 0.2); border: 2rpx solid rgba(255, 152, 0, 0.4); }
...
.objective-item {
  display: grid;
  grid-template-columns: 48rpx 1fr auto 120rpx;
  align-items: center;
  gap: 0.5rem;
  font-size: 0.8rem;
}
...
.status-dot.active {
  background: #4ade80;
  box-shadow: 0 0 16rpx rgba(74, 222, 128, 0.5);
...
.ai-advice-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(640rpx, 1fr));
  gap: 1.25rem;
  margin-bottom: 2rem;
}
...
.advice-card.combat { border-left: 6rpx solid #f44336; }
.advice-card.deck { border-left: 6rpx solid #4caf50; }
.advice-card.resource { border-left: 6rpx solid #ffd700; }
.advice-card.strategy { border-left: 6rpx solid #9c27b0; }
...
.stress-modal-container {
  width: min(2048rpx, 95vw);
  max-height: 94vh;
  background: rgba(13, 17, 23, 0.95);
  border-radius: 32rpx;
  border: 2rpx solid rgba(59, 130, 246, 0.3);
  box-shadow: 0 40rpx 120rpx rgba(0, 0, 0, 0.6);
  display: flex;
  flex-direction: column;
  overflow: hidden;
}
...
.stress-summary-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(440rpx, 1fr));
  gap: 1.25rem;
  margin-bottom: 1.5rem;
}
...
.stress-level-card {
  ...
  border-radius: 24rpx;
  padding: 1rem 1.25rem;
  border: 2rpx solid rgba(55, 65, 81, 0.8);
}
...
.debuff-row.mental { border-left: 6rpx solid #9c27b0; }
.debuff-row.combat { border-left: 6rpx solid var(--error); }
.debuff-row.behavioral { border-left: 6rpx solid var(--warning); }
...
@keyframes pulse-green {
  ...
  70% { box-shadow: 0 0 0 20rpx rgba(76, 175, 80, 0); }
...
@media (max-width: 2048rpx) {
  .main-content {
    grid-template-columns: minmax(520rpx, 600rpx) 1fr;
  }
...
@media (max-width: 1536rpx) {
  ...
  .objective-item {
    grid-template-columns: 48rpx 1fr 120rpx;
    gap: 0.25rem;
  }
...
@media (max-width: 960rpx) {
  ...
  .objective-item {
    grid-template-columns: 40rpx 1fr 100rpx;
    gap: 0.25rem;
  }
}
</style>