<template>
  <view class="stress-indicator">
    ...
    <view class="stress-info">
      <text class="stress-level-text">{{ stressLevelText }}</text>
    ...
    <!-- 压力详情弹窗 -->
    <view v-if="showDetails" class="stress-details">
      <h4>压力状态详情</h4>
      <view class="stress-stats">
        <view class="stat-item">
          <text>当前压力值:</text>
          <text>{{ currentStress }}/100</text>
        </view>
        <view class="stat-item">
          <text>压力等级:</text>
          <text :class="stressLevelClass">{{ stressLevelText }}</text>
        </view>
      </view>
      
      <!-- 激活的debuff -->
      <view v-if="activeDebuffs.length > 0" class="debuffs-section">
        <h5>当前负面效果</h5>
        <view class="debuffs-list">
          <view 
            ...
            :class="debuff.type"
          >
            <view class="debuff-icon">
              <i :class="getDebuffIcon(debuff.type)"></i>
            </view>
            <view class="debuff-info">
              <view class="debuff-name">{{ debuff.name }}</view>
              <view class="debuff-description">{{ debuff.description }}</view>
            </view>
            <view class="debuff-chance">{{ Math.round(debuff.triggerChance * 100) }}%</view>
          </view>
        </view>
      </view>
      
      <!-- 缓解建议 -->
      <view class="stress-relief-suggestions">
        <h5>缓解建议</h5>
        <view class="suggestions-list">
          ...
          >
            ...
            <view class="suggestion-info">
              ...
              <view class="suggestion-effect">-{{ suggestion.reliefAmount }} 压力</view>
            </view>
            <view class="suggestion-cost">
