...
<template>
  <main class="battle-field">
    ...
    <view class="battle-arena">
      <!-- 我方区域（左） -->
      <view class="ally-zone" :class="{ 'ally-turn-active': turn === 'player' }">
        <!-- 左侧竖排信息区 -->
        <view class="vertical-info-panel ally-panel">
          <view class="vertical-info-content">
            <!-- 圆形头像框 -->
            <view class="player-avatar-container" :class="{ 'hit-effect': playerHitEffect }">
        ...
        <view class="battle-area">
          <view class="battle-slots">
            <!-- 左列：位置 3, 4, 5 -->
            <view class="battle-column left-column">
              <view
              ...
              >
                <!-- 攻击时的克隆体（跳出方框，使用固定定位，避免任何裁剪/层级影响） -->
                <view 
              ...
              <view v-else class="empty-slot">
                <text class="slot-hint">位置 {{ slotIndex + 3 }}</text>
              </view>
              </view>
            </view>
            <!-- 右列：位置 0, 1, 2 -->
            <view class="battle-column right-column">
              <view
                v-for="slotIndex in 3"
                :key="slotIndex - 1"
                class="battle-slot"
              ...
              <view v-else class="empty-slot">
                <text class="slot-hint">位置 {{ slotIndex }}</text>
              </view>
              </view>
            </view>
          </view>
      ...
      <!-- 中央分割 -->
      <view class="battle-divider vertical">
      ...
      <!-- 敌方区域（右） -->
      <view class="enemy-zone" :class="{ 'enemy-turn-active': turn === 'opponent' }">
        <!-- 中间对战区 -->
        <view class="battle-area">
          <view class="battle-slots">
            <!-- 左列：位置 0, 1, 2 -->
        ...
        <!-- 右侧竖排信息区 -->
        <view class="vertical-info-panel enemy-panel">
          <view class="vertical-info-content">
            <!-- 圆形头像框 -->
            <view 
...
/* 卡牌本体保持裁剪，限制内部光效不出界（克隆体使用固定定位，不受此限制） */
.character-card {
  overflow: hidden;
}
